...
REFRESH_INTERVAL = int(os.getenv("REFRESH_INTERVAL", 86400))  # default 24h
...
# Helpers for endpoints/backtest to fetch existing cached datasets
def fetch_daily():
    if DAILY_FILE.exists():
        try:
            ...
            print("⚠️ fetch_daily: read error:", e)
    return pd.DataFrame()
...
def fetch_hourly():
    if HOURLY_FILE.exists():
        try:
            ...
            print("⚠️ fetch_hourly: read error:", e)
    return pd.DataFrame()

# -----------------------
# Extended TwelveData batch fetchers (kept)
# (fetch_twelvedata_xauusd and fetch_twelvedata_xauusd_daily are unchanged)
# -----------------------
def fetch_twelvedata_xauusd(api_key, interval="1h", total_records=35000):
    print(f"📡 Fetching up to {total_records} rows of {interval} data from TwelveData...")
    base_url = "https://api.twelvedata.com/time_series"
    end_date = datetime.utcnow()
    all_data = []
    batch_size = 5000
    batches = max(1, total_records // batch_size)
    ...
    if "datetime" in df.columns:
        df["datetime"] = pd.to_datetime(df["datetime"])
    df = df.sort_values(by="datetime" if "datetime" in df.columns else df.columns[0])
    print(f"🎯 Final merged dataset: {len(df)} rows of {interval} XAU/USD data.")
    return df

def fetch_twelvedata_xauusd_daily(api_key, total_records=12000):
...
# -----------------------
# Volume profile, FVG, regime, sl/tp, ensemble etc.
# -----------------------
...
def regime_detector(df):
    ...
    if vol < 0.005 and trend:
        return "low-vol-trend"
    if vol < 0.005 and not trend:
...
def ensemble_predict_proba(df_latest):
    try:
        ...
        if any(f not in df_latest.columns for f in features):
            return None
        ...
        print("⚠️ ensemble_predict_proba error:", e)
        return None
...
def build_train_and_signal():
    ...
    try:
        ...
        hourly_df = pd.read_csv(HOURLY_FILE) if HOURLY_FILE.exists() else pd.DataFrame()
    ...
    if (daily_df.empty or hourly_df.empty) and MODEL_PATH.exists():
        # attempt to use cached CSVs and saved ensemble
        try:
            if DAILY_FILE.exists() and HOURLY_FILE.exists():
                d = pd.read_csv(DAILY_FILE)
                h = pd.read_csv(HOURLY_FILE)
                d = d.rename(columns=lambda x: x.capitalize())
                h = h.rename(columns=lambda x: x.capitalize())
                proc_d = compute_indicators(d)
                prob_d = ensemble_predict_proba(proc_d)
                fused_d = fuse_signal(prob_d, proc_d)
                proc_h = compute_indicators(h)
            ...
            print("⚠️ Offline regeneration failed:", e)
    ...
    try:
        ...
        print("⚠️ Could not write signals file:", e)
    ...
    try:
        ...
        print("⚠️ Could not write history file:", e)
...
def backtest_route():
    ...
    results = []
    window = 200
    for i in range(window, max(window + 1, len(proc) - 10), 10):
        train = proc.iloc[:i]
...
def refresh_signals():
    ...
    This endpoint triggers build_train_and_signal() and returns the produced signals.
    Request body: optional JSON (ignored). Example: {}
    """
    try:
        combined = build_train_and_signal()
        return jsonify({"status": "✅ Signal regenerated", "data": combined}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500

# -----------------------
# Background auto-refresh loop (runs in daemon thread)
# -----------------------
def auto_refresh_loop():
    while True:
        try:
            print("🔄 Auto-refresh signals (background)...")
...
# Background refresh (run once on start)
def background_loop_once():
    try:
        print("🚀 Running initial build and signal generation...")
        build_train_and_signal()
        print("✅ Initial signal generation complete — check /signal endpoint.")
    except Exception as e:
...
if __name__ == "__main__":
    # run initial refresh and start the background thread for periodic refreshes
    t_init = threading.Thread(target=background_loop_once, daemon=True)
    t_init.start()

    t_auto = threading.Thread(target=auto_refresh_loop, daemon=True)
    t_auto.start()

    # Ensure we have at least cached CSVs if present
    if DAILY_FILE.exists() and HOURLY_FILE.exists():
        try:
            df = pd.read_csv(DAILY_FILE)
            ...
            print("⚠️ Cache load failed, will re-fetch on demand.", e)
    else:
        print("📥 No cached CSV found (will fetch on first /signal call)")
